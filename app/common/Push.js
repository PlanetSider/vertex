...
const ServerChan = require('../libs/push/serverchan');
const MeoW = require('../libs/push/meow');
...
class Push {
  constructor (push) {
    ...
    const additionPushType = [
      ...
      'pushTelegram', 'pushNtfy', 'pushWebhook', 'pushServerChan', 'pushMeoW'
    ];
  ...
  async doRequest (type, args) {
    ...
    try {
      if (type === 'push') {
        return await (this.p.pushWeChat || this.p.pushSlack || this.p.pushTelegram || this.p.pushNtfy || this.p.pushWebhook || this.p.pushServerChan || this.p.pushMeoW)(...args);
      }
  ...
  async pushServerChan (...args) {
    await this.doRequest('pushServerChan', args);
  }

  async pushMeoW (...args) {
    await this.doRequest('pushMeoW', args);
  }
