...
class MeoW {
  constructor (meow) {
    this.meowNickname = meow.meowNickname;
    this.meowUrl = meow.meowUrl || 'https://api.chuckfang.com';
    this.meowMsgType = meow.meowMsgType || 'text';
    this.meowHtmlHeight = meow.meowHtmlHeight;
    this.alias = meow.alias;
  };

  async pushMeoW (title, desp) {
    const url = `${this.meowUrl}/${encodeURIComponent(this.meowNickname)}`;
    const params = {
      title,
      msg: desp
    };
    if (this.meowMsgType) {
      params.msgType = this.meowMsgType;
    }
    if (this.meowHtmlHeight && this.meowMsgType === 'html') {
      params.htmlHeight = this.meowHtmlHeight;
    }
    const option = {
      url,
      method: 'POST',
    ...
    if (statusCode !== 200) {
      logger.error('MeoW推送失败', this.alias, title, res.body);
      return;
    }
    const result = JSON.parse(res.body);
    if (result.status !== 200) {
      logger.error('MeoW推送失败', this.alias, title, result.message || result.msg);
      return;
    }
    return result;
  };
  ...
  async rssError (rss) {
    ...
    await this.pushMeoW(title, desp);
  };
  ...
  async connectClient (client) {
    ...
    const desp = `当前时间: ${moment().format('YYYY-MM-DD HH:mm:ss')}\n` +
      `下载器名: ${client.alias}`;
    return await this.pushMeoW(title, desp);
  };
