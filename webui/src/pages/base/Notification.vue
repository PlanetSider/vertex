      ...
      <a-form
        ...
        <a-form-item
          ...
          <a-select size="small" v-model:value="notification.type"  >
            ...
            <a-select-option value="meow">MeoW</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item
        ...
        <a-form-item
          v-if="notification.type === 'serverchan'"
          label="SendKey"
          name="sendkey"
          extra="Server酱³ 的 SendKey，支持 sctp 开头的 SC3 格式和旧版 SCT 格式"
          :rules="[{ required: true, message: '${label}不可为空! ' }]">
          <a-input size="small" v-model:value="notification.sendkey"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'serverchan'"
          label="标签"
          name="serverchanTags"
          extra="SC3专用：标签列表，多个标签使用竖线分隔">
          <a-input size="small" v-model:value="notification.serverchanTags"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'serverchan'"
          label="简短描述"
          name="serverchanShort"
          extra="消息卡片的简短描述，适用于 Markdown 内容">
          <a-input size="small" v-model:value="notification.serverchanShort"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'serverchan'"
          label="推送通道"
          name="serverchanChannel"
          extra="SCT专用：推送使用的消息通道，多个通道用竖线隔开">
          <a-input size="small" v-model:value="notification.serverchanChannel"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'serverchan'"
          label="OpenID"
          name="serverchanOpenid"
          extra="SCT专用：消息抄送的 openid，用于企业微信应用消息">
          <a-input size="small" v-model:value="notification.serverchanOpenid"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'serverchan'"
          label="隐藏调用IP"
          name="serverchanNoip">
          <a-switch v-model:checked="notification.serverchanNoip" />
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'meow'"
          label="昵称"
          name="meowNickname"
          extra="MeoW 用户昵称"
          :rules="[{ required: true, message: '${label}不可为空! ' }]">
          <a-input size="small" v-model:value="notification.meowNickname"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'meow'"
          label="API地址"
          name="meowUrl"
          extra="默认: https://api.chuckfang.com">
          <a-input size="small" v-model:value="notification.meowUrl"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'meow'"
          label="消息类型"
          name="meowMsgType"
          extra="text 为纯文本，html 为 HTML 格式渲染">
          <a-select size="small" v-model:value="notification.meowMsgType" style="width: 120px;">
            <a-select-option value="text">text</a-select-option>
            <a-select-option value="html">html</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'meow' && notification.meowMsgType === 'html'"
          label="HTML高度"
          name="meowHtmlHeight"
          extra="App 中显示 HTML 内容的高度（像素），默认 200">
          <a-input-number size="small" v-model:value="notification.meowHtmlHeight" :min="100" :max="1000" />
        </a-form-item>
        <a-form-item
          label="推送类型"
          name="pushType"
